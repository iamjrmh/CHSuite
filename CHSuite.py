...
class StyledDropdown(tk.Canvas):
    ...
    def __init__(self, parent, textvariable=None, values=None,
        ...
        # Use Press + Release so first-click reliability matches AppKit on
        # macOS (where ButtonPress on a fresh Canvas is sometimes consumed
        # by window-focus handling).
...
class RoundedButton(tk.Canvas):
    ...
    def __init__(self, parent, text, command, bg_color, hover_color,
        ...
        # macOS Tk drops the first <Button-1> on a fresh Canvas; bind
        # press + release so a click lands on either edge of the gesture.
        self._press_inside = False
        self.bind("<ButtonPress-1>",   self._on_press)
        self.bind("<ButtonRelease-1>", self._on_release)
        self.bind("<Enter>",     self._on_enter)
...
class _RoundedNavBtn(tk.Canvas):
    ...
    def __init__(self, parent, icon, label, command, canvas_bg, **kw):
        ...
        self._active  = False
        self._hovered = False
        # macOS Tk frequently drops the first <Button-1> on a Canvas widget
        # (the press is consumed by window-focus handling instead).  Track
        # press + release explicitly and fire on release if the press landed
        # inside the button — this is also how AppKit dispatches clicks, so
        # it's the most native-feeling behaviour cross-platform.
        self._press_inside = False
    ...
    def _release(self, e):
        if not self._press_inside:
            return
        self._press_inside = False
        # Only fire if the pointer is still inside the button — drag-out
        # cancels are a standard "actually I didn't mean it" gesture.
        try:
            w = self.winfo_width()
            h = self.winfo_height()
        except Exception:
...
class CHSuite(
    ...
    def __init__(self):
        ...
        # ── Splash screen ─────────────────────────────────────────────────────
        # Total perceived duration ≈ 10 s.  Heavy work (preload all tabs +
        # disk scan) is scheduled via self.after(...) so it runs INSIDE
        # mainloop, where the asset-scan worker thread can safely use
        # self.after(0, ...) to deliver results back to the UI.  Doing the
        # work synchronously in __init__ would race with Python 3.10+'s
        # "main thread is not in main loop" check and lose scan results.
        import time as _time
        self._splash_started_at = _time.monotonic()
        self._splash_widget = self._show_splash()
        ...
        SPLASH_TOTAL_MS = 10000

        # Schedule preload + initial profile/scan once mainloop is running.
        # The 50 ms delay lets the splash render and start its fade-in
        # before we begin the (potentially heavy) preload.
        def _run_heavy():
            try:
                self._preload_all_pages(splash=self._splash_widget)
            except Exception as ex:
                _log(f"[preload] failed: {ex}")
            try:
                self._load_initial_profile()
            except Exception as ex:
                _log(f"[init] _load_initial_profile failed: {ex}")
        self.after(50, _run_heavy)

        # Schedule splash dismissal at the 10-second mark.  Anchor the
        # timer to _splash_started_at so the user always sees ~10 s of
        # splash regardless of how long the synchronous bits of __init__
        # took (build_ui, fade-in, etc.).
        def _dismiss():
            try:
                self._dismiss_splash(self._splash_widget)
            except Exception as ex:
                _log(f"[splash] dismiss failed: {ex}")
            try:
                self.deiconify()
            except Exception:
                pass
        elapsed_ms = int((_time.monotonic() - self._splash_started_at) * 1000)
        remaining_ms = max(0, SPLASH_TOTAL_MS - elapsed_ms)
        self.after(remaining_ms, _dismiss)

        # ── ThemeGen IPC server ───────────────────────────────────────────
        self._start_themegen_ipc()

        # ── Discord Rich Presence (deferred so the IPC connect doesn't
        #    delay first paint when Discord is offline / unreachable) ─────────
        _client_id = self._cfg.get("discord_client_id", DISCORD_CLIENT_ID_DEFAULT)
        # Stub so any code path that touches self._drpc before it's ready
        # silently no-ops instead of AttributeError'ing.
        class _DiscordStub:
            _running = False
            def update(self, *a, **k): pass
            def close(self): pass
        self._drpc = _DiscordStub()
        def _drpc_connect():
            try:
                drpc = _DiscordRPC(_client_id)
            except Exception:
                return
            # Hop back to the Tk thread before touching widgets / state.
            def _apply():
                self._drpc = drpc
                try:
            ...
            try:
                ...
                pass
        threading.Thread(target=_drpc_connect, daemon=True).start()
    ...
    def _apply_styles(self):
        ...
        # ── Scrollbars ────────────────────────────────────────────────────────
        # Keep the clam theme's default layout (trough + arrows + thumb) so
        # the scrollbar stays interactive and clearly visible — only restyle
        # colors so it matches the dark theme.  An earlier attempt redefined
        # the layout to drop the arrows but rendered an unusable / invisible
        # scrollbar on some platforms.
        for _orient in ("Vertical.TScrollbar", "Horizontal.TScrollbar"):
            s.configure(_orient,
                        background=C["accent_dim"], troughcolor=C["panel"],
                        bordercolor=C["panel"],
                        lightcolor=C["accent_dim"],
                        darkcolor=C["accent_dim"],
                        arrowcolor=C["text_mid"],
        ...
        # Same palette for legacy tk.Scrollbar (used inside StyledDropdown
        # popups).  option_add only applies to widgets that don't set these
        # explicitly, so existing call sites keep their per-instance colours.
        self.option_add("*Scrollbar.background",          C["accent_dim"])
        self.option_add("*Scrollbar.troughColor",         C["panel"])
        self.option_add("*Scrollbar.activeBackground",    C["accent"])
        ...
        # ── Checkbutton (used on Settings + elsewhere) ───────────────────────
        # Default Tk on Windows draws an invisible checkmark on a dark bg.
        # Force a white checkmark and accent-tinted indicator background so
        # the state is unambiguous regardless of theme.
        self.option_add("*Checkbutton.foreground",        C["text"])
        self.option_add("*Checkbutton.activeForeground",  C["text"])
        self.option_add("*Checkbutton.selectColor",       C["accent"])
    ...
    # ── navigation sidebar ────────────────────────────────────────────────────
    # Master tool list — single source of truth used by both _build_nav and
    # the Settings page tool-visibility editor.  "about" and "settings" are
    # excluded from the visibility editor (always shown).
    _NAV_ITEMS = (
        ("about",        "ⓘ", "About This Tool"),
        ("bgchanger",    "◈", "CHMenuChanger"),
        ("namegen",      "✦", "CHNameGen"),
    ...
    _ALWAYS_VISIBLE = ("about",)

    def _tool_visibility(self) -> dict:
        """Return the saved tool-visibility map, defaulting all on."""
        defaults = {pid: True for pid, _, _ in self._NAV_ITEMS
                    if pid not in self._ALWAYS_VISIBLE}
        saved = self._cfg.get("tool_visibility", {})
        for k in defaults:
    ...
    def _build_nav(self):
        ...
        # Build all tool buttons (filtered by tool-visibility config).  Buttons
        # are kept in self._nav_btns so the Settings page can show/hide them
        # live by toggling pack/pack_forget without rebuilding the nav.
        vis = self._tool_visibility()
        for page_id, icon, label in self._NAV_ITEMS:
            ...
            if page_id in self._ALWAYS_VISIBLE or vis.get(page_id, True):
                btn.pack(fill="x", padx=8, pady=2)
            self._nav_btns[page_id] = btn

        # ── Settings button (above the version footer) ───────────────────────
        # Theme picker, Check for Updates, and Reset to Default now live on
        # the Settings page; this single nav button is their entry point.
        # Flexible spacer above the Settings button so it sticks to the bottom.
        # _nav_spacer is stored so _apply_tool_visibility can re-pack tools
        # above it (preserving the bottom-anchored Settings button).
        self._nav_spacer = tk.Frame(self._nav, bg=C["sidebar"])
        self._nav_spacer.pack(fill="y", expand=True)

        settings_sep = tk.Frame(self._nav, bg=C["border"], height=1)
        settings_sep.pack(fill="x", padx=12, pady=(0, 6))

        settings_btn = _RoundedNavBtn(
            self._nav, "❖", "Settings",
            command=lambda: self._show_page("settings"),
            canvas_bg=C["sidebar"],
        )
        settings_btn.pack(fill="x", padx=8, pady=(0, 6))
    ...
    # ── tab preload (runs during splash) ──────────────────────────────────────
    def _preload_all_pages(self, splash=None):
        """Build every page that hasn't been built yet so first-click on any
        sidebar tab is instant.  Called once during the startup splash;
        already-built pages (about, bgchanger) are skipped.  splash.update()
        is called between page builds so the splash stays painted even
        while heavy widget construction blocks the main thread."""
        page_ids = [pid for pid, _i, _l in self._NAV_ITEMS] + ["settings"]
        for page_id in page_ids:
            ...
            try:
                ...
                _log(f"[preload] {page_id} failed: {exc}")
            try:
    ...
    def _show_splash(self):
        """Show a centred Toplevel splash with fade-in.  Returns the Toplevel
        (or None on failure).  Uses per-pixel transparency on Windows (and
        transparent root on macOS where supported) so the PNG's alpha channel
        is honoured.  Falls back to the embedded base64 icon when
        Images/JURMRWEED.png is not present so the app never blocks on it."""
        try:
            splash = tk.Toplevel(self)
            splash.overrideredirect(True)
            # Transparency key: any pixel painted in this exact colour becomes
            # fully transparent on Windows.  Use a near-black tone that is
            # extremely unlikely to appear inside a real image.
            TKEY = "#010001"
            splash.configure(bg=TKEY)
            try:
                splash.attributes("-topmost", True)
            except Exception:
                pass
            # Per-pixel transparency.  Only Windows truly honours this; on
            # macOS/Linux the splash falls back to the TKEY background, which
            # is fine because the PNG itself fills the splash area.
            _transparent_ok = False
            if sys.platform == "win32":
                try:
                    splash.wm_attributes("-transparentcolor", TKEY)
                    _transparent_ok = True
                except Exception:
                    pass
            elif sys.platform == "darwin":
                try:
                    splash.wm_attributes("-transparent", True)
                    splash.configure(bg="systemTransparent")
                    _transparent_ok = True
                except Exception:
                    pass
            try:
            ...
            png_path = _resources_dir() / "Images" / "JURMRWEED.png"
            try:
                from PIL import Image as _Img, ImageTk as _ImgTk
                if png_path.is_file():
                    pil = _Img.open(png_path)
                else:
                    import base64 as _b64, io as _io
                    pil = _Img.open(_io.BytesIO(_b64.b64decode(_ICON_B64)))
                # Force RGBA so the alpha channel is preserved when handed to
                # PhotoImage.  When the OS supports per-pixel transparency the
                # image's alpha shows through directly; otherwise the
                # transparent regions are flattened onto TKEY (which is itself
                # mostly invisible anyway thanks to the alpha-fade).
                if pil.mode != "RGBA":
                    pil = pil.convert("RGBA")
                # Cap to a sensible splash size (preserve aspect ratio)
                MAX_DIM = 480
                if max(pil.size) > MAX_DIM:
                    ...
                    pil = pil.resize(
                        ...
                        _Img.LANCZOS)
                img = _ImgTk.PhotoImage(pil)
                splash._splash_img = img  # keep ref alive
                lbl_bg = ("systemTransparent"
                          if (_transparent_ok and sys.platform == "darwin")
                          else TKEY)
                tk.Label(splash, image=img, bg=lbl_bg,
                         borderwidth=0, highlightthickness=0).pack()
            except Exception:
                tk.Label(splash, text="CHSuite",
                         font=("Lato", 24, "bold"),
                         fg="#ffffff", bg=TKEY,
                         padx=80, pady=40).pack()

            # Centre on screen
            splash.update_idletasks()
            sw, sh = splash.winfo_screenwidth(), splash.winfo_screenheight()
            w = max(splash.winfo_reqwidth(), 1)
            h = max(splash.winfo_reqheight(), 1)
            ...
            for i in range(steps + 1):
                try:
                    splash.attributes("-alpha", i / steps)
                except Exception:
                    pass
                try:
                    splash.update()
    ...
    def _finish_splash(self, splash, min_duration_ms: int = 2400):
        """Hold the splash until min_duration_ms has elapsed (counted from
        self._splash_started_at), then fade out and destroy.  Used only
        when the splash needs to be torn down inside __init__ (not the
        new mainloop-driven path)."""
        if splash is None:
            return
        import time as _t
        try:
            target = getattr(self, "_splash_started_at", _t.monotonic()) + (
    ...
    def _dismiss_splash(self, splash):
        """Fade the splash out and destroy it.  Safe to call from inside
        the Tk mainloop (e.g. via self.after())."""
        if splash is None:
            return
        import time as _t
        try:
    ...
    # ── tool-visibility (driven by Settings page) ─────────────────────────────
    def _apply_tool_visibility(self):
        """Show/hide nav buttons based on saved tool-visibility config."""
        if not hasattr(self, "_nav_btns"):
            return
        vis = self._tool_visibility()
        spacer = getattr(self, "_nav_spacer", None)
        for page_id, _, _ in self._NAV_ITEMS:
            ...
            try:
                ...
                pass
            if page_id in self._ALWAYS_VISIBLE or vis.get(page_id, True):
                kwargs = {"fill": "x", "padx": 8, "pady": 2}
                if spacer is not None:
                    kwargs["before"] = spacer
                btn.pack(**kwargs)
        # If the active page got hidden, fall back to About.
        if (self._current_page and
                self._current_page not in self._ALWAYS_VISIBLE and
                self._current_page != "settings" and
                not vis.get(self._current_page, True)):
            self._show_page("about")
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  PAGE — SETTINGS (Theme · Updates · Reset · Tool Visibility)
    # ══════════════════════════════════════════════════════════════════════════
    def _build_page_settings(self):
        ...
        tk.Label(inner,
                 text="Configure how CHSuite looks, updates, and which tools appear in the sidebar.",
                 font=FT, fg=C["text_dim"], bg=C["bg"], wraplength=820,
                 justify="left").pack(anchor="w", pady=(2, 6))
        _sep(inner, bg=C["border"]).pack(fill="x", pady=(0, 12))
        ...
        def _section(title, subtitle=""):
            ...
            tk.Label(head, text=title, font=("Lato", 13, "bold"),
                     fg=C["text"], bg=C["card"]).pack(anchor="w")
            if subtitle:
                tk.Label(head, text=subtitle, font=("Lato", 9),
        ...
        theme_body = _section(
            "Theme",
            "Pick a colour theme.  Changes take effect after a quick restart.")
        # Initialize the var lazily — it used to live on the nav.
        if not hasattr(self, "_theme_var"):
            self._theme_var = tk.StringVar(
                value=self._cfg.get("theme", "Default"))
        theme_row = tk.Frame(theme_body, bg=C["card"])
        theme_row.pack(fill="x", pady=(0, 8))
        self._theme_cb = StyledDropdown(
            theme_row, textvariable=self._theme_var,
            values=_list_themes(), state="readonly",
            font=("Lato", 9), width=24, canvas_bg=C["card"], height=34)
        self._theme_cb.pack(side="left")
        self._theme_cb.bind("<<ComboboxSelected>>", self._on_theme_change)
        RoundedButton(
            theme_row, "🎨  Theme Lab", self._open_theme_lab,
            bg_color=C["accent_dim"], hover_color=C["accent"],
            text_color=C["text"], height=34, radius=12,
            text_font=("Lato", 9, "bold"), canvas_bg=C["card"], width=140,
        ).pack(side="left", padx=(10, 0))

        # ── Updates section ──────────────────────────────────────────────────
        upd_body = _section(
            "Updates",
            "Check GitHub for newer CHSuite builds and install them in place.")
        RoundedButton(
            upd_body, "↑  Check for Updates", self._check_for_updates,
            bg_color=C["accent_dim"], hover_color=C["accent"],
            text_color=C["text"], height=38, radius=14,
            text_font=("Lato", 10, "bold"), canvas_bg=C["card"], width=220,
        ).pack(anchor="w")

        # ── Reset section ────────────────────────────────────────────────────
        reset_body = _section(
            "Reset to Default",
            "Delete all CHSuite-created config and profile files, returning the "
            "app to a fresh-install state.  CHSuite will close after the reset.")
        RoundedButton(
            reset_body, "⟳  Reset to Default", self._reset_to_default,
            bg_color="#3d1a1a", hover_color=C["error"],
            text_color=C["error"], height=38, radius=14,
            text_font=("Lato", 10, "bold"), canvas_bg=C["card"], width=220,
        ).pack(anchor="w")
        ...
        vis_body = _section(
            "Sidebar Tools",
            "Toggle which tools appear in the left navigation.  About and "
            "Settings are always shown.")

        vis = self._tool_visibility()
        self._tool_vis_vars: dict = {}
        for page_id, icon, label in self._NAV_ITEMS:
            if page_id in self._ALWAYS_VISIBLE:
                continue
            row = tk.Frame(vis_body, bg=C["card"])
            row.pack(fill="x", pady=2)
            var = tk.BooleanVar(value=bool(vis.get(page_id, True)))
            self._tool_vis_vars[page_id] = var
            def _make_toggle(pid=page_id, v=var):
                def _on():
                    saved = self._cfg.get("tool_visibility", {}) or {}
                    saved[pid] = bool(v.get())
                    self._cfg["tool_visibility"] = saved
                    _save_json(CONFIG_FILE, self._cfg)
                    self._apply_tool_visibility()
                return _on
            chk = tk.Checkbutton(
                row, variable=var, command=_make_toggle(),
            ...
            tk.Label(row, text=f"{icon}   {label}",
                     font=("Lato", 10), fg=C["text"], bg=C["card"]).pack(
                         side="left", padx=(4, 0))
    ...
    def _check_for_updates(self):
                ...
                def _mac_apply():
                    try:
                        ...
                        if fname_lower.endswith(".pkg"):
                            ...
                            import plistlib
                            # Auto-accept any SLA / EULA inside the DMG so the
                            # mount never hangs waiting for an interactive
                            # "Agree" prompt that has no UI in this context.
                            ...
                            if mount_result.returncode != 0:
                                raise RuntimeError(
                                    ...
                                    f"{mount_result.stderr.strip() or 'unknown error'}")
                            try:
                                plist_data = plistlib.loads(
                                    mount_result.stdout.encode())
                            except Exception:
                                # Some hdiutil builds prepend a SLA banner
                                # before the plist; trim to the first '<'.
                                _stdout = mount_result.stdout
                                _idx = _stdout.find("<?xml")
                                plist_data = (plistlib.loads(_stdout[_idx:].encode())
                                              if _idx >= 0 else {})
                            mount_point = None
                            ...
                            try:
                                ...
                                if not apps:
                                    # Some DMGs nest the .app one level deep.
                                    apps = list(Path(mount_point).glob("*/*.app"))
                                if not apps:
                                    raise RuntimeError(
                                ...
                                # ditto preserves extended attributes / code
                                # signatures when copying an .app bundle.
                                # Fall back to shutil.copytree if ditto fails.
                                try:
                                    subprocess.run(
                                        ["ditto", str(src_app), str(dst_app)],
                                        check=True, timeout=180)
                                except Exception:
                                    shutil.copytree(str(src_app), str(dst_app))
                                # Strip the quarantine attribute so Gatekeeper
                                # doesn't block the freshly-installed bundle
                                # on first launch ("can't be opened because
                            ...
                            # Portable / unsigned macOS build distributed as a
                            # zip containing CHSuite.app.  Extract and copy
                            # the .app to /Applications (or to the running
                            # location if that's not /Applications).
                            import zipfile
                            def _start_zip():
                                prog_bar.config(mode="indeterminate")
                                prog_bar.start(15)
                                prog_lbl.config(text="Extracting archive…")
                                prog_frame.pack(anchor="w", fill="x", pady=(8, 0))
                                win.geometry(f"390x290+{x}+{y}")
                                status_var.set("Extracting archive…")
                                status_lbl.config(fg=C["text_mid"])
                                close_btn.config(state="disabled")
                            self.after(0, _start_zip)
                            staging = Path(tmp_dir) / "staging"
                            staging.mkdir(parents=True, exist_ok=True)
                            with zipfile.ZipFile(str(tmp_file), "r") as zf:
                                zf.extractall(staging)
                            apps = list(staging.glob("*.app")) + list(
                                staging.glob("*/*.app"))
                            if not apps:
                                raise RuntimeError(
                                    "No .app bundle found inside the zip.")
                            src_app = apps[0]
                            # Pick destination: if running from /Applications
                            # use that, otherwise update next to the running
                            # bundle so portable installs get refreshed too.
                            if (getattr(sys, "frozen", False) and
                                    str(_run_dir).lower().startswith(
                                        "/applications")):
                                dst_app = Path("/Applications") / src_app.name
                            else:
                                dst_app = (Path(_run_dir).parent / src_app.name
                                           if Path(_run_dir).name == "MacOS"
                                           else Path(_run_dir) / src_app.name)
                                # Default to /Applications if we can't infer
                                # a better place (typical for first-run from
                                # ~/Downloads).
                                if not str(dst_app).startswith(("/Applications",
                                                                str(Path.home()))):
                                    dst_app = Path("/Applications") / src_app.name
                            if dst_app.exists():
                                shutil.rmtree(str(dst_app))
        ...
        def _fetch():
            ...
            if _IS_LINUX:
                ...
                # macOS asset selection — prefer .dmg first (auto-mounts via
                # hdiutil and copies the .app bundle into /Applications with
                # no sudo prompt).  Order:
                #   1. Any .dmg whose name mentions mac/macos/darwin
                #   2. Any .dmg at all (release naming is sometimes generic)
                #   3. .pkg (sudo required, but works)
                #   4. .zip with mac/macos/darwin in the name (portable .app)
                # Each candidate is filtered through _name_filter so we don't
                # accidentally pick a Windows or Linux artifact.
                def _name_match(a, *needles):
                    n = a.get("name", "").lower()
                    return any(s in n for s in needles)
                def _ext_match(a, ext):
                    return a.get("name", "").lower().endswith(ext)
                asset = (
                    next((a for a in assets
                          if _ext_match(a, ".dmg") and
                             _name_match(a, "mac", "darwin", "osx")), None)
                    or next((a for a in assets if _ext_match(a, ".dmg")), None)
                    or next((a for a in assets
                             if _ext_match(a, ".pkg") and
                                _name_match(a, "mac", "darwin", "osx")), None)
                    or next((a for a in assets if _ext_match(a, ".pkg")), None)
                    or next((a for a in assets
                             if _ext_match(a, ".zip") and
                                _name_match(a, "mac", "darwin", "osx")), None)
                )
            elif is_portable:
                asset = next(
            ...
            def _show():
                ...
                if remote <= local:
                    ...
                    # Surface what assets WERE on the release so users can tell
                    # the maintainer "the .dmg is missing" instead of seeing
                    # an opaque "no matching asset" message.
                    avail = ", ".join(a.get("name", "?") for a in assets) or "none"
                    status_var.set(
                        f"v{tag} is available — no matching asset for this "
                        f"platform.  Release contains: {avail}")
                    status_lbl.config(fg=C["warn"])
            self.after(0, _show)
