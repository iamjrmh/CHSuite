...
# Split mixin files — all must be present beside CHSuite.py
for mixin in CHMenuChanger.py CHNameGen.py CHNoteGen.py CHCleaner.py CHPatcher.py CHSongManager.py CHManager.py; do
    [[ -f "$mixin" ]] || fail "$mixin not found. All split mixin files must be beside CHSuite.py."
done

# base64.txt — bundled into _internal/ at build time so load_base64() works frozen
[[ -f "base64.txt" ]] || fail "base64.txt not found. It contains embedded icon/font data and must be beside CHSuite.py."
