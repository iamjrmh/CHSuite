...
const state = {
  mc: { assetBytes: null, assetName: null, imgBytes: null, imgName: null },
  ng: { mode: "gradient" },
...
async function initPyodide() {
  ...
  try {
    ...
    const [ws, rt, cs] = await Promise.all([
      fetch("py/write_spec.py").then(r => r.text()),
      fetch("py/rthook_texture2d.py").then(r => r.text()),
      fetch("py/CHSuiteLite.py").then(r => r.text()),
    ]);
...
setupDropZone(mcAssetDrop, mcAssetInput, mcAssetPill, ".assets", async file => {
  const buf = await file.arrayBuffer();
  state.mc.assetBytes = new Uint8Array(buf);
  state.mc.assetName  = file.name;

  if (pyReady) {
    await populateTextureList();
  } else {
    toast("Python not ready yet — please wait.");
  }
  updateMcButton();
});

setupDropZone(mcImgDrop, mcImgInput, mcImgPill, "image/*", async file => {
  const buf = await file.arrayBuffer();
...
function updateMcButton() {
  mcProcessBtn.disabled = !(state.mc.assetBytes && state.mc.imgBytes && mcTextureSelect.value);
}

mcTextureSelect.addEventListener("change", updateMcButton);

async function populateTextureList() {
  if (!pyReady || !state.mc.assetBytes) return;
  try {
    pyodide.globals.set("_asset_bytes", state.mc.assetBytes);
    const namesJson = pyodide.runPython(`
import sys
list_textures(bytes(_asset_bytes.to_py()))
`);
    const names = JSON.parse(namesJson);
    if (names.error) { toast("⚠ Could not read textures: " + names.error); return; }
    ...
    mcTextureRow.style.display = "";
    toast(`Found ${names.length} texture(s).`);
  } catch (err) {
    console.error(err);
    toast("⚠ UnityPy may not be available in Pyodide. Try uploading a simple asset.");
  }
}

mcProcessBtn.addEventListener("click", async () => {
  if (!pyReady) { toast("Python not ready."); return; }
  const texName = mcTextureSelect.value;
  if (!texName || !state.mc.assetBytes || !state.mc.imgBytes) return;

  mcProcessBtn.disabled = true;
  mcProcessBtn.textContent = "Processing…";

  try {
    pyodide.globals.set("_asset_bytes", state.mc.assetBytes);
    pyodide.globals.set("_img_bytes",   state.mc.imgBytes);
    pyodide.globals.set("_tex_name",    texName);

    const result = pyodide.runPython(`
result_bytes = process_menu(bytes(_asset_bytes.to_py()), bytes(_img_bytes.to_py()), _tex_name)
import js; js.globalThis._pyResult = result_bytes
result_bytes
`);
    const outBytes = result.toJs();

    state.mc._resultBytes = new Uint8Array(outBytes);
    mcOutput.classList.remove("hidden");
    toast("✓ Texture replaced successfully!");
  } catch (err) {
    console.error(err);
    toast("⚠ Failed to process asset: " + err.message, 4000);
  } finally {
    mcProcessBtn.disabled = false;
    mcProcessBtn.innerHTML = `<svg viewBox="0 0 20 20" fill="none" width="16" height="16"><path d="M4 12l2-2 3 3 5-6 2 2-7 8-5-5z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/></svg> Apply Texture`;
    updateMcButton();
  }
});
