...
for mixin in CHMenuChanger.py CHNameGen.py CHNoteGen.py CHCleaner.py CHPatcher.py CHSongManager.py CHManager.py; do
    if [[ ! -f "$mixin" ]]; then
        echo " [ERROR] $mixin not found. All split mixin files must be beside CHSuite.py."
        exit 1
    fi
done
...
if [[ ! -f "base64.txt" ]]; then
    echo " [ERROR] base64.txt not found. It contains embedded icon/font data and must be beside CHSuite.py."
    exit 1
fi
