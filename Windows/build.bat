...
:: ── Split mixin files (required by the split-build architecture) ──────────────
for %%M in (
    CHMenuChanger.py
    CHNameGen.py
    CHNoteGen.py
    ...
    if not exist "%%M" (
        echo  [ERROR] %%M not found in this folder.
        echo  All split mixin files must be beside CHSuite.py.
        goto :fail
    )
)

if not exist "base64.txt" (
    echo  [ERROR] base64.txt not found in this folder.
    echo  base64.txt contains the embedded icon and font data and must be
    echo  beside CHSuite.py so it can be bundled into _internal\.
    goto :fail
)
